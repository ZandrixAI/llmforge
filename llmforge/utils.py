...
def _unpack_awq_weights(qweight: torch.Tensor) -> torch.Tensor:
    bits = 4
    pack_factor = 32 // bits
    out_features, packed_in = qweight.shape
    in_features = packed_in * pack_factor
    mask = (1 << bits) - 1  # e.g., 0xF for 4-bit
    shifts = torch.tensor([0, 4, 1, 5, 2, 6, 3, 7]) * bits
    shifts = shifts.to(qweight.device)
...
def _transform_awq_weights(
    ...
    for key in list(weights.keys()):
        if key.endswith(".g_idx"):
            raise ValueError(
                ...
                "or re-quantize the model using llmforge.convert."
            )
        ...
        if key.endswith(".qweight"):
            ...
            shifts = torch.arange(pack_factor) * bits
            shifts = shifts.to(repacked.device)
            weight = (repacked.to(torch.uint32) << shifts).sum(axis=-1).to(torch.uint32)
            ...
            if qzeros_key in weights:
                ...
                biases = -unpacked_zeros.to(torch.float32) * scales
            else:
                zero_point = 1 << (bits - 1)
                biases = (
                    torch.full(scales.shape, -zero_point, dtype=torch.float32) * scales
                )

            new_weights[f"{prefix}.weight"] = weight
            new_weights[f"{prefix}.scales"] = scales
            new_weights[f"{prefix}.biases"] = biases.to(scales.dtype)
    ...
    for k, w in new_weights.items():
        if torch.is_floating_point(w):
            new_weights[k] = w.to(model_dtype)
...
def get_total_parameters(model):
    ...
    for name, module in model.named_modules():
        if hasattr(module, "bits"):
            n = 0 if not hasattr(module, "bias") else module.bias.numel()
            total += n + module.weight.numel() * 32 // module.bits
        elif list(module.children()) == [] or module is model:
...
def compute_bits_per_weight(model):
    model_bytes = sum(p.element_size() * p.nelement() for p in model.parameters())
...
def load_model(
    ...
    def _quantize(quantization):
        # Placeholder: quantization is handled separately in PyTorch
        pass
    ...
    if config.get("quantize_activations", False):
        # Placeholder: activation quantization kept as stub
        pass
...
def sharded_load(
    ...
    from .distributed import init as dist_init
    ...
    if pipeline_group is tensor_group is None:
        if has_tensor_parallel:
            tensor_group = dist_init()
        elif has_pipelining:
            pipeline_group = dist_init()
    ...
    if tensor_group is not None and tensor_group.size() > 1:
        model.shard(tensor_group)
    if pipeline_group is not None and pipeline_group.size() > 1:
...
def pipeline_load(repo, return_config=False):
    ...
    return sharded_load(repo, dist_init(), None, return_config)
...
def make_shards(weights: dict, max_file_size_gb: int = MAX_FILE_SIZE_GB) -> list:
    ...
    for k, v in weights.items():
        if shard_size + v.element_size() * v.nelement() > max_file_size_bytes:
            shards.append(shard)
            shard, shard_size = {}, 0
        shard[k] = v
        shard_size += v.element_size() * v.nelement()
    shards.append(shard)
    return shards
...
def create_model_card(path: Union[str, Path], hf_path: Union[str, Path, None]):
    ...
    card.data.library_name = "pytorch"
    card.data.pipeline_tag = "text-generation"
    if card.data.tags is None:
        card.data.tags = ["pytorch"]
    elif "pytorch" not in card.data.tags:
        card.data.tags += ["pytorch"]
...
def upload_to_hub(path: str, upload_repo: str):
    ...
    if hf_path is not None:
        ...
        converted to PyTorch format from [{hf_path}](https://huggingface.co/{hf_path})
        using llmforge version **{__version__}**.
        """
    else:
        provenance = ""
    ...
    card.text = dedent(
        ...
        ```python
        from llmforge import load, generate
...
def save_model(
    ...
    *,
    ...
    total_size = sum(v.element_size() * v.nelement() for v in weights.values())
    index_data = {
        "metadata": {
    ...
    for i in range(len(shards)):
        ...
        safetensors.torch.save_file(shard, str(shard_path))
...
def quantize_model(
    ...
    # Placeholder: quantization kept separate in PyTorch
    # nn.quantize(
    #     model,
    #     group_size,
    #     bits,
...
def dequantize_model(model: nn.Module) -> nn.Module:
    ...
    """
    # Stub: dequantization is not applicable in PyTorch conversion
    return model
