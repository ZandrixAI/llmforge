"""
LLMForge - Evaluation module for LLM models.
...
class MLXLM(LM):
    ...
    def _score_fn(self, inputs, cache: Optional[Any] = None, step_size: int = 2048):
        ...
        for i in range(0, inputs.shape[1], step_size):
            ...
            score = torch.gather(
                log_probs, -1, targets[:, i : i + step_size].unsqueeze(-1)
            ).squeeze(-1)

            ig = targets[:, i : i + step_size] == torch.argmax(logits, dim=-1)
            ig = torch.where(
                torch.arange(offset, T + offset) < lengths[:, None], ig, False
            )
        ...
        is_greedy = torch.cat(is_greedy, dim=1)
    ...
    def loglikelihood(self, requests) -> list[tuple[float, bool]]:
        ...
        for q, rs in tqdm(zip(questions, responses), total=len(questions)):
            ...
            max_idx = torch.argmax(logprobs).item()
            ...
            for s in full_sequences:
                ...
                score, _, ig = self._score_fn(
                    torch.tensor(inputs).unsqueeze(0), cache=copy.deepcopy(cache)
                )
                scores[-1] += torch.sum(score).item()
                is_greedy[-1] &= torch.all(ig).item()
        ...
        per_group = group.all_max(torch.tensor(len(scores))).item()
        scores = scores + [0] * (per_group - len(scores))
        is_greedy = is_greedy + [False] * (per_group - len(is_greedy))
        scores = torch.tensor(scores)
        is_greedy = torch.tensor(is_greedy)
        scores = group.all_gather(scores)
        is_greedy = group.all_gather(is_greedy)
        ...
        inv_sort = torch.argsort(torch.tensor(all_indices))
    ...
    def loglikelihood_rolling(self, requests) -> list[float]:
        ...
        for i in tqdm(range(0, len(inputs), self._batch_size)):
            ...
            mask = torch.arange(scores.shape[-1]) < lengths[:, None]
            all_scores.extend((mask * scores).sum(dim=-1).tolist())
    ...
    def generate_until(self, requests) -> list[str]:
        ...
        if group.size() > 1:
            ...
            max_len = torch.tensor(max(len(c) for c in completions))
            max_len = group.all_max(max_len).item()
            lengths = torch.tensor([len(c) for c in completions] + [0] * pad)
            completions = torch.tensor(
                [c + [0] * (int(max_len) - len(c)) for c in completions]
                + [[0] * int(max_len)] * pad,
                dtype=torch.uint8,
            )
            completions = (
                group.all_gather(completions.unsqueeze(0))
                .swapaxes(0, 1)
                .flatten(0, 1)
                .tolist()
            )
            lengths = (
                group.all_gather(lengths.unsqueeze(0))
                .swapaxes(0, 1)
                .flatten(0, 1)
                .tolist()
            )
