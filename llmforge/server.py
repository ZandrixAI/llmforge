...
from .utils import _parse_size, load


def get_system_fingerprint():
    gpu_info = torch.cuda.get_device_name(0) if torch.cuda.is_available() else "cpu"
    return f"{__version__}-{torch.__version__}-{platform.platform()}-{gpu_info}"
    ...
    def __next__(self):
        ...
        if self._current_iterations > self._iterations:
            ...
            if self._loops % self._sync_frequency == 0:
                loop_time = self._time_spent
                avg_loop_time = loop_time / (1 * self._sync_frequency)
...
def _format_top_logprobs(logprobs, top_logprobs, tokenizer) -> Tuple[Dict[str, Any]]:
    ...
    if top_logprobs <= 0:
        return ()
    sorted_indices = torch.topk(-logprobs, top_logprobs).indices
            ...
            if tokenizer.has_chat_template:
                ...
                if tools and not tokenizer.has_tool_calling:
                    logging.warning(
                        ...
                        "https://github.com/llmforge/llmforge/issues"
                    )
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def handle_chat_completions(self) -> CompletionRequest:
        ...
        Returns:
            torch.Tensor: The tokenized prompt from the request body
        """
        body = self.body
    ...
    def handle_models_request(self):
        ...
        downloaded_models = [
            repo for repo in hf_cache_info.repos if probably_llmforge(repo)
        ]
...
def _run_http_server(
    ...
    warnings.warn(
        "llmforge.server is not recommended for production as "
...
if __name__ == "__main__":
    print(
        "Calling `python -m llmforge.server...` directly is deprecated."
        " Use `llmforge.server...` or `python -m llmforge server ...` instead."
    )
    main()
