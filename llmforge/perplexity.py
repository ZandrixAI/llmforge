"""
LLMForge - Evaluate perplexity (PPL) of LLM models.
"""
...
from llmforge.tuner.datasets import load_dataset
from llmforge.utils import get_total_parameters, load
...
def load_data(
    ...
    data = torch.tensor(data[: (len(data) // sequence_length) * sequence_length])
...
def eval_ppl(model, data, batch_size=8):
    ...
    for i, s in enumerate(range(0, len(data), batch_size)):
        ...
        logits = model(batch[:, :-1]).to(torch.float32)

        # Calculate cross-entropy loss with next tokens
        losses = nn.functional.cross_entropy(logits, batch[:, 1:], reduction="none")
    ...
    std_dev = torch.sqrt(torch.var(all_losses, unbiased=True)).item()
...
def main():
    ...
    parser.add_argument(
        ...
        help="A Hugging Face dataset which is compatible with an llmforge dataset format.",
    )
    parser.add_argument(
