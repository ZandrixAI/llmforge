...
LLMForge has several options for reducing quality loss from quantization:
...
```
pip install "llmforge[train]"
```

### DWQ

Use `llmforge.dwq` to run DWQ on a given model. For example:

```bash
llmforge.dwq --model Qwen/Qwen3-0.6B
```

Some important options, along with their default values are:

- `--model-path llmforge_model`: The location to save the DWQ model.
...
Use `llmforge.dynamic_quant` to generate a dynamic quantization of given model.
For example:

```bash
llmforge.dynamic_quant --model Qwen/Qwen3-0.6B
```
...
Use `llmforge.awq` to run AWQ on a given model. For example:

```bash
llmforge.awq --model Qwen/Qwen3-0.6B
```
...
- `--model-path llmforge_model`: The location to save the AWQ model.
- `--bits 4`: Precision of the quantization.
...
Use `llmforge.gptq` to run GPTQ on a given model. For example:

```bash
llmforge.gptq --model Qwen/Qwen3-0.6B
```
...
model on downstream tasks using `llmforge.evaluate`. For example:

```bash
llmforge.evaluate \
    --model llmforge_model \
    --tasks winogrande boolq arc_challenge arc_easy hellaswag openbookqa piqa social_iqa                     
```

### Upload to Hugging Face

Use `llmforge.upload` to upload the quantized model to the Hugging Face Hub. For
example:

```bash
llmforge.upload \
    --path llmforge_model \
    --upload-repo hf-user/Mistral-7B-Instruct-v0.3-3bit-DWQ
```
