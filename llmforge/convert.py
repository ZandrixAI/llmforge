...
from .models.utils import tree_map_with_path
...
def convert(
    ...
    if dtype in MODEL_CONVERSION_DTYPES:
        ...
        def set_dtype(k, v):
            if cast_predicate(k) and v.dtype.is_floating_point:
                return v.to(dtype)
            else:
                return v

        for name, param in model.named_parameters():
            new_param = set_dtype(name, param)
            if new_param.data_ptr() != param.data_ptr():
...
if __name__ == "__main__":
    print(
        "Calling `python -m llmforge.convert ...` directly is deprecated."
        " Use `llmforge.convert ...` or `python -m llmforge convert ...` instead."
    )
    main()
