...
from .memory import (
    ...
    get_peak_memory,
    is_available as _gpu_available,
    model_memory_bytes,
    synchronize,
)
...
def tree_reduce(fn, tree, initializer=None):
    ...
    if isinstance(tree, (list, tuple)):
        for item in tree:
            initializer = fn(initializer, item)
    return initializer
...
def wired_limit(model: nn.Module, streams: Optional[List] = None):
    """
    A context manager to manage GPU memory during generation.

    Mirrors MLX's mx.set_wired_limit behavior:
    - Clears GPU cache before generation
    - Warns if model is close to GPU memory limit
    - Synchronizes before/after if streams are provided
    """
    if _gpu_available():
        _clear_cache()
        model_bytes = model_memory_bytes(model)
        total_mem = 0
        try:
            ...
            pass
        if total_mem > 0 and model_bytes > 0.9 * total_mem:
            model_mb = model_bytes // 2**20
            total_mb = total_mem // 2**20
            print(
                f"[WARNING] Generating with a model that requires {model_mb} MB "
                f"which is close to the maximum recommended size of {total_mb} MB. "
                "This can be slow."
            )
...
class GenerationResponse:
    ...
    Args:
        ...
        logprobs (torch.Tensor): A vector of log probabilities.
...
def generate_step(
    ...
    *,
    ...
    Args:
        ...
        sampler (Callable[torch.Tensor, torch.Tensor], optional): A sampler for sampling a
          token from a vector of log probabilities. Default: ``None``.
        logits_processors (List[Callable[[torch.Tensor, torch.Tensor], torch.Tensor]], optional):
        ...
        quantized_kv_start (int): Token offset to start quantizing KV cache.
        prompt_progress_callback (Callable, optional): Callback for prompt progress.
        input_embeddings (torch.Tensor, optional): Input embeddings.
        stop_token_ids (List[int], optional): Token IDs that stop generation.
        stop_words (List[str], optional): Words that stop generation when detected.
          Requires ``tokenizer`` to be provided.
        tokenizer: Tokenizer for stop_words decoding.
        prefill_step_size (int): Step size for processing the prompt.
        ...
        input_embeddings (torch.Tensor, optional): Input embeddings to use instead of or in
          conjunction with prompt tokens. Default: ``None``.

    Yields:
        Tuple[torch.Tensor, torch.Tensor]: One token and a vector of log probabilities.
    """
    ...
    sampler = sampler or (lambda x: torch.argmax(x, dim=-1))

    def _model_call(
        input_tokens: torch.Tensor, input_embeddings: Optional[torch.Tensor]
    ):
        if input_embeddings is not None:
            return model(
    ...
    def _step(
        input_tokens: torch.Tensor, input_embeddings: Optional[torch.Tensor] = None
    ):
        nonlocal tokens
    ...
    generated_tokens = []
    generated_text = ""
    stop_token_set = set(stop_token_ids) if stop_token_ids else set()
    while True:
        if n != max_tokens:
        ...
        # Track generated tokens/text for stop word detection
        generated_tokens.append(token_id)
        if stop_token_set and token_id in stop_token_set:
            break
        if stop_words and tokenizer:
            generated_text = tokenizer.decode(generated_tokens, errors="replace")
            if any(sw in generated_text for sw in stop_words):
                break
...
def speculative_generate_step(
    ...
    *,
    ...
    Yields:
        Tuple[torch.Tensor, torch.Tensor, bool]: One token, a vector of log probabilities,
          and a bool indicating if the token was generated by the draft model
    """

    y = prompt.to(torch.int32)
    prev_tokens = None
    ...
    def _step(model, cache, y, n_predict=1):
        ...
        if logits_processors:
            ...
            for i in range(n_predict):
                prev_tokens = (
                    torch.cat([prev_tokens, y]) if prev_tokens is not None else y
                )
                y, logprobs = _process_and_sample(prev_tokens, logits[:, i, :])
                out_y.append(y)
                out_logprobs.append(logprobs)
            return torch.cat(out_y, dim=0), torch.cat(out_logprobs, dim=0)
        else:
    ...
    try:
        while True:
            ...
            y = torch.tensor([tokens[n]], dtype=torch.int32)
            draft_y = y
            ...
            if n == num_draft:
                draft_y = torch.cat(
                    [torch.tensor(draft_tokens[-1:], dtype=torch.int32), draft_y]
                )
...
def stream_generate(
    ...
    **kwargs,
    ...
    Args:
        ...
        prompt (Union[str, torch.Tensor, List[int]]): The input prompt string or
          integer tokens.
...
def _left_pad_prompts(prompts, max_length=None):
    ...
    return torch.tensor([[0] * (max_length - len(p)) + p for p in prompts])


def _right_pad_prompts(prompts, max_length=None):
    if max_length is None:
        max_length = max(len(p) for p in prompts)
    return torch.tensor([p + [0] * (max_length - len(p)) for p in prompts])
...
class Batch:
    ...
    def filter(self, keep_idx: List[int]):
        ...
        keep_idx = torch.tensor(keep_idx, dtype=torch.int32)
...
def _make_cache(model, left_padding, max_kv_size):
    ...
    def to_batch_cache(c):
        if type(c) is KVCache:
            ...
            c.left_padding = torch.tensor(left_padding)
            return c
...
class BatchGenerator:
    ...
    def __init__(
        ...
        self.sampler = sampler or (lambda x: torch.argmax(x, dim=-1))
    ...
    def _process_prompts(self, prompts):
        ...
        tokens = [torch.tensor(inp) for inp in inputs]
        processed_tokens = 0
        ...
        #   3. Finalize the KV caches so they are left padded again
            ...
            last_inputs = torch.tensor([p[-prompt_checkpoint:] for p in inputs])
    ...
    def _step(
        ...
        if any(samplers):
            ...
            sampled = torch.cat(all_samples, dim=0)
        else:
    ...
    def _next(self):
        ...
        for i, toks in enumerate(batch.tokens):
            batch.tokens[i] = torch.cat((toks, y[i : i + 1]))
...
if __name__ == "__main__":
    print(
        "Calling `python -m llmforge.generate...` directly is deprecated."
        " Use `llmforge.generate...` or `python -m llmforge generate ...` instead."
    )
    main()
