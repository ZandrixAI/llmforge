...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.attn_sub_norm = RMSNorm(args.hidden_size, eps=args.rms_norm_eps)
...
class MLP(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.ffn_sub_norm = RMSNorm(args.intermediate_size, eps=args.rms_norm_eps)

    def forward(self, x) -> torch.Tensor:
        x = torch.square(F.relu(self.gate_proj(x))) * self.up_proj(x)
