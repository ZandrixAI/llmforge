...
def swiglu_oss(x_linear, x_glu, alpha: float = 1.702, limit: float = 7.0):
    x_glu = torch.clamp(x_glu, max=limit)
    x_linear = torch.clamp(x_linear, min=-limit, max=limit)
    glu_scaled = alpha * x_glu
    sig = torch.sigmoid(glu_scaled)
    out_glu = x_glu * sig
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor, cache=None) -> torch.Tensor:
        B, L, _ = x.shape
        D = self.head_dim
        Hk = self.num_key_value_heads

        q = self.q_proj(x).reshape(B, L, -1, D).transpose(1, 2)
        k = self.k_proj(x).reshape(B, L, -1, D).transpose(1, 2)
        v = self.v_proj(x).reshape(B, L, -1, D).transpose(1, 2)
        ...
        return self.o_proj(v_hat.transpose(1, 2).reshape(B, L, -1))
...
class MLPBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        g = self.router(x)
        experts, indices = torch.topk(g, k=self.num_experts_per_tok, dim=-1)
        expert_weights = torch.softmax(experts, dim=-1)

        # Experts block
        x = self.experts(x, indices)

        x = x * expert_weights.unsqueeze(-1)
    ...
    def sanitize(self, weights):
        ...
        for k, v in weights.items():
            if "gate_up_proj" in k and "bias" not in k:
                if "_blocks" in k:
                    v = v.view(torch.uint32).flatten(-2)
                    k = k.replace("_blocks", ".weight")
                if "_scales" in k:
                    k = k.replace("_scales", ".scales")
                new_weights[k.replace("gate_up_proj", "gate_proj")] = v[
                    ..., ::2, :
                ].contiguous()
                new_weights[k.replace("gate_up_proj", "up_proj")] = v[
                    ..., 1::2, :
                ].contiguous()
