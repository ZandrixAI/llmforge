...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.q_norm = RMSNorm(n_heads * head_dim, eps=args.rms_norm_eps)
        self.k_norm = RMSNorm(n_kv_heads * head_dim, eps=args.rms_norm_eps)
...
class OlmoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        k = self.top_k
        indices = torch.topk(-routing_weights, k=k, dim=-1).indices.detach()
        scores = torch.gather(routing_weights, -1, indices)
