...
def gelu_topk(inputs, std_multiplier):
    ...
    cutoff_x = inputs_mean + inputs_std * std_multiplier.to(inputs_std.dtype)
    return F.gelu(torch.relu(inputs - cutoff_x), approximate="tanh")
...
class MLP(nn.Module):
    def __init__(self, config: TextConfig, layer_idx: int = 0):
        ...
        if self.activation_sparsity > 0:
            self._std_multiplier = math.sqrt(2.0) * torch.erfinv(
                torch.tensor(2 * self.activation_sparsity - 1)
            )
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.activation_sparsity > 0.0:
            ...
            activations = F.gelu(gate_proj, approximate="tanh")
...
class Gemma3nAltUp(nn.Module):
    ...
    def compute_router_modalities(self, x: torch.Tensor) -> torch.Tensor:
        ...
        routed = self.modality_router(router_inputs).to(torch.float32)
        return torch.tanh(routed)

    def predict(self, x: torch.Tensor) -> torch.Tensor:
        modalities = self.compute_router_modalities(x[self.config.altup_active_idx])

        weight = self.prediction_coefs.weight.to(torch.float32)
        ...
        x_up = x.to(torch.float32)
        x_permuted = x_up.permute(1, 2, 3, 0)
        predictions = torch.matmul(x_permuted, all_coefs)
        predictions = predictions.permute(3, 0, 1, 2)
        predictions += x_up
        return predictions.to(x.dtype)

    def correct(self, predictions: torch.Tensor, activated: torch.Tensor):
        modalities = self.compute_router_modalities(activated)

        weight = self.correction_coefs.weight.to(torch.float32)
...
class LanguageModel(nn.Module):
    ...
    def forward(
        ...
        target_magnitude = torch.mean(h0**2, dim=-1, keepdim=True) ** 0.5

        h_list = [h0]
        h_list.extend([proj(h0) for proj in self.altup_projections])
        h = torch.stack(h_list, dim=0)
        mags = torch.mean(h[1:] ** 2, dim=-1, keepdim=True) ** 0.5
        h[1:] = h[1:] * (
            target_magnitude / torch.clamp(mags, min=torch.finfo(h0.dtype).min)
        )
        ...
        target_magnitude = torch.mean(h[0] ** 2, dim=-1, keepdim=True) ** 0.5
        ...
        out = self.norm(h)
        out = F.linear(out, self.embed_tokens.weight)
