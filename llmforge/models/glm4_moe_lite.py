        ...
        for l in range(self.args.num_hidden_layers):
            ...
            if f"{prefix}.kv_b_proj.weight" in weights:
                ...
                if quantized:
                    ...
                    bits = (v.shape[-1] * 32) // self.args.kv_lora_rank
                    group_size = self.args.kv_lora_rank // scales.shape[-1]
                    from .quantization import dequantize

                    v = dequantize(v, scales, biases, bits=bits, group_size=group_size)
                ...
                if quantized:
                    ...
                    wk, wk_scales, wk_biases = quantize(
                        wk, bits=bits, group_size=group_size
                    )
                    wv, wv_scales, wv_biases = quantize(
