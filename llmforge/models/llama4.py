...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if self.use_qk_norm:
            # Manual RMSNorm without learnable weight
            queries = queries * torch.rsqrt(
                queries.pow(2).mean(-1, keepdim=True) + 1e-6
            )
            keys = keys * torch.rsqrt(keys.pow(2).mean(-1, keepdim=True) + 1e-6)
...
class LlamaModel(nn.Module):
    ...
    def forward(
        ...
        linds = torch.arange(start, end, device=h.device)
        rinds = torch.arange(offset, end, device=h.device)[:, None]
        block_pos = torch.abs(
    ...
    def sanitize(self, weights):
        ...
        for l in range(self.args.text_config.num_hidden_layers):
            ...
            if f"{prefix}.gate_up_proj" in weights:
                ...
                gate_proj, up_proj = torch.split(v, v.shape[-1] // 2, dim=-1)
                weights[gate_k] = gate_proj.transpose(-1, -2)
                weights[up_k] = up_proj.transpose(-1, -2)
            if f"{prefix}.down_proj" in weights:
                down_proj = weights.pop(f"{prefix}.down_proj")
                weights[f"{prefix}.down_proj.weight"] = down_proj.transpose(-1, -2)
        return weights
