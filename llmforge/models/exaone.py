...
class AttentionModule(nn.Module):
    ...
    def forward(
        ...
        k = self.k_proj(x).reshape(B, L, self.n_kv_heads, -1).transpose(1, 2)
        v = self.v_proj(x).reshape(B, L, self.n_kv_heads, -1).transpose(1, 2)
...
class ExaoneModel(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.h = nn.ModuleList([TransformerBlock(args) for _ in range(args.num_layers)])
...
class Model(nn.Module):
    ...
    def forward(
        ...
        if self.args.tie_word_embeddings:
            out = F.linear(out, self.transformer.wte.weight)
        else:
