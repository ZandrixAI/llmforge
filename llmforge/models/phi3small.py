...
def gegelu_impl(a_gelu, a_linear, limit):
    a_gelu = torch.where(
        torch.isinf(a_gelu),
        a_gelu,
        torch.clamp(a_gelu, max=limit),
    )
    a_linear = torch.where(
        torch.isinf(a_linear),
        a_linear,
        torch.clamp(a_linear, min=-limit, max=limit),
    )
...
class Attention(nn.Module):
    ...
    def _block_sparse_mask(self, q_len, kv_len):
        ...
        q_pos = torch.arange(kv_blocks - q_blocks, kv_blocks)[None, :, None]
        k_pos = torch.arange(kv_blocks)[None, None]

        mask_vert_strided = (
            torch.arange(kv_blocks)[None, :] + torch.arange(1, n_heads + 1)[:, None]
        ) % vert_stride
        ...
        dense_mask = torch.repeat_interleave(
            torch.repeat_interleave(block_mask, block_size, dim=-1), block_size, dim=-2
        )
    ...
    def _block_sparse_attention(self, queries, keys, values, scale, mask):
        ...
        B = queries.shape[0]
        L = queries.shape[2]
        queries = torch.reshape(queries, (B, self.n_kv_heads, self.n_q_per_kv, L, -1))
        ...
        scores = scores + torch.where(
            ...
            torch.tensor(float("-inf"), dtype=scores.dtype),
        )
        scores = torch.softmax(scores, dim=-1)

        output = scores @ values
        return torch.reshape(output, (B, self.n_heads, L, -1))
    ...
    def forward(
        ...
        if cache is not None:
            queries = cache.offset_apply(queries, cache.offset)
            keys = cache.offset_apply(keys, cache.offset)
...
class Model(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self._dummy_tokenizer_ids = torch.tensor(
