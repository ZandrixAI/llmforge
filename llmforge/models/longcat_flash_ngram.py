...
class NgramEmbedding(nn.Module):
    ...
    def _shift_right(self, x: torch.Tensor, n: int) -> torch.Tensor:
        if n <= 0:
            return x
        ...
        return torch.cat(
            [torch.zeros((batch_size, n), dtype=x.dtype, device=x.device), x[..., :-n]],
            dim=-1,
        )
    ...
    def forward(
        ...
        if cache is not None:
            ...
            if context is None:
                ...
                context = torch.cat([context, input_ids], dim=-1)
...
class LongcatFlashNgramModel(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.layers = nn.ModuleList(
            [LongcatFlashDecoderLayer(args) for _ in range(args.num_layers)]
        )
