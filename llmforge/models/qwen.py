...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, cache=None):
        ...
        queries = q.reshape(B, L, self.num_attention_heads, -1).transpose(1, 2)
        keys = k.reshape(B, L, self.num_attention_heads, -1).transpose(1, 2)
        values = v.reshape(B, L, self.num_attention_heads, -1).transpose(1, 2)
