...
class Attention(nn.Module):
    ...
    def forward(
        ...
        keys = keys.reshape(B, L, self.n_head, -1).transpose(1, 2)
        values = values.reshape(B, L, self.n_head, -1).transpose(1, 2)
...
class GPT2Model(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.h = nn.ModuleList(
            [TransformerBlock(args=args) for _ in range(self.n_layer)]
        )
    ...
    def forward(
        ...
        position_ids = torch.arange(L) + offset[..., None]
...
class Model(nn.Module):
    ...
    def forward(
        ...
        out = F.linear(out, self.model.wte.weight)
        return out
