...
def clip_residual(x, y):
    if x.dtype != torch.float16:
        return x + y
    bound = torch.finfo(torch.float16).max
    return torch.clamp(x.to(torch.float32) + y.to(torch.float32), -bound, bound).to(
        torch.float16
    )
...
class Gemma3Model(nn.Module):
    ...
    def forward(
        ...
        h *= torch.tensor(self.args.hidden_size**0.5, dtype=torch.bfloat16).to(h.dtype)
