...
class ModelArgs(BaseModelArgs):
    ...
    moe_capacity: list = field(default_factory=list)
    moe_k: int = 1
...
class Ernie4_5_MoeMLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        gates = self.gate(x)
        gates = self.gate_act(gates.to(torch.float32))

        k = self.k
        _, inds = torch.topk(-gates, k, dim=-1)
        scores = torch.gather(gates, -1, inds)

        scores = scores / torch.clamp(scores.sum(dim=-1, keepdim=True), min=1e-12)
        ...
        for l in range(self.args.num_hidden_layers):
            ...
            for m in ["gate_proj", "down_proj", "up_proj"]:
                if f"{prefix}.mlp.experts.0.{m}.weight" in weights:
                    ...
                    weights[f"{prefix}.mlp.switch_mlp.{m}.weight"] = torch.stack(
                        to_join
                    )
