...
def compute_mup_vector(args):
    ...
    return torch.cat([torch.full((s,), m) for s, m in zip(sizes, args.ssm_multipliers)])
...
class FalconH1Mixer(nn.Module):
    ...
    def forward(self, input_states, cache=None, mask: Optional[torch.Tensor] = None):
        ...
        gate, conv_input, dt = torch.split(
            ...
            [
                ...
                projected_states.shape[-1] - self.intermediate_size - self.conv_dim,
            ],
            dim=-1,
        )
...
class FalconH1DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        mamba_mask: Optional[torch.Tensor],
    ) -> torch.Tensor:
...
class FalconH1Model(nn.Module):
    def __init__(self, args):
        ...
        self.final_layernorm = RMSNorm(self.hidden_size, eps=args.rms_norm_eps)
...
class Model(nn.Module):
    ...
    def forward(self, inputs, cache=None):
        ...
        if self.args.tie_word_embeddings:
            out = F.linear(hidden_states, self.model.embed_tokens.weight)
        ...
        for name, param in weights.items():
            ...
            if name.endswith("embed_tokens.weight"):
                ...
                param *= (
                    ...
                    * self.model._mup_vector.to(param.dtype)[:, None]
                )
