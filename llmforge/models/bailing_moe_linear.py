...
def recurrent_gla(
    ...
    outputs = []
    exp_g = torch.exp(g)[:, None, None].to(q.dtype)
    q = q * scale
    for t in range(L):
        q_t = q[:, :, t : t + 1]
        k_t = k[:, :, t : t + 1]
        v_t = v[:, :, t : t + 1]
        h_up = k_t.transpose(2, 3) @ v_t
        if h is not None:
            h = h * exp_g + h_up
        else:
            h = h_up
...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        if args.use_qk_norm:
            self.key_layernorm = RMSNorm(self.head_dim, eps=args.rms_norm_eps)
            self.query_layernorm = RMSNorm(self.head_dim, eps=args.rms_norm_eps)
    ...
    def forward(
        ...
        keys = k.reshape(B, L, self.num_key_value_heads, self.head_dim).transpose(1, 2)
...
class LinearAttention(nn.Module):
    ...
    def forward(
        ...
        output = output.transpose(1, 2).reshape(B, L, -1)
        output = self.g_norm(output) * torch.sigmoid(self.g_proj(x))
        return self.dense(output)
...
def group_expert_select(
    ...
    if score_function == "sigmoid":
        ...
        scores = F.softmax(gates.to(torch.float32), dim=-1)
    orig_scores = scores
...
class SparseMoeBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = (out * topk_weight[..., None]).sum(dim=-2)
...
class Model(nn.Module):
    ...
    def forward(
        ...
        if self.args.tie_word_embeddings:
            out = F.linear(out, self.model.word_embeddings.weight)
        else:
    ...
    def sanitize(self, weights):
        ...
        if self.norm_head:
            ...
            dtype = w.dtype
            weight_norm = torch.norm(w.to(torch.float32), dim=0, keepdim=True) + 1e-7
            weights["lm_head.weight"] = (w / weight_norm).to(dtype)
