...
class JambaMambaMixer(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        A = torch.arange(1.0, self.ssm_state_size + 1.0).reshape(1, self.ssm_state_size)
        A = A.repeat(self.intermediate_size, 1)
        ...
        self.dt_layernorm = RMSNorm(self.time_step_rank, eps=args.rms_norm_eps)
        self.b_layernorm = RMSNorm(self.ssm_state_size, eps=args.rms_norm_eps)
        self.c_layernorm = RMSNorm(self.ssm_state_size, eps=args.rms_norm_eps)
    ...
    def ssm_step(self, x, A, state=None):
        ...
        new_state = (delta * x).unsqueeze(-1) * B.unsqueeze(-2)
        dtA = torch.exp((delta.unsqueeze(-1) * A))
...
class JambaDecoderLayer(nn.Module):
    def __init__(self, args: ModelArgs, layer_type: str, layer_idx: int):
        ...
        self.pre_ff_layernorm = RMSNorm(args.hidden_size, eps=args.rms_norm_eps)
...
class JambaModel(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.final_layernorm = RMSNorm(args.hidden_size, eps=args.rms_norm_eps)
        ...
        for l in range(self.args.num_hidden_layers):
            ...
            for proj in ["gate_proj", "down_proj", "up_proj"]:
                for name in ["weight", "bias", "scales", "biases"]:
                    ...
                    if expert_tensors:
                        weights[f"{base}.switch_mlp.{proj}.{name}"] = torch.stack(
                            expert_tensors
                        )
