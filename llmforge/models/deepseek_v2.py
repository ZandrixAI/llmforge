...
class DeepseekV2YarnRotaryEmbedding(nn.Module):
    def __init__(
        ...
        freq_extra = base ** (torch.arange(0, dim, 2, dtype=torch.float32) / dim)
...
class DeepseekV2Attention(nn.Module):
    ...
    def forward(
        ...
        if cache is not None:
            ...
            k_pe = k_pe.repeat(1, self.num_heads, 1, 1)
            keys, values = cache.update_and_fetch(
                torch.cat([k_nope, k_pe], dim=-1), values
            )
        else:
            q_pe = self.rope(q_pe)
            k_pe = self.rope(k_pe)
...
class MoEGate(nn.Module):
    ...
    def forward(self, x):
        ...
        scores = torch.softmax(gates, dim=-1)

        if self.topk_method == "group_limited_greedy":
            bsz, seq_len = x.shape[:2]
            scores = scores.reshape(bsz, seq_len, self.n_group, -1)
            group_scores = scores.max(dim=-1, keepdim=True).values
