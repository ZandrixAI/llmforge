...
def clamped_swiglu(x, gate, limit):
    gate = torch.clamp(F.silu(gate), max=limit)
...
def moe_gate_select(gates, router_bias, top_k, routed_scaling_factor, norm_topk_prob):
    ...
    topk_indices = torch.topk(corrected_scores, k=top_k, dim=-1).indices
...
class Step3p5Attention(nn.Module):
    ...
    def forward(
        ...
        queries = self.q_norm(queries.reshape(B, L, self.num_heads, -1)).transpose(1, 2)
        keys = self.k_norm(keys.reshape(B, L, self.num_kv_heads, -1)).transpose(1, 2)
