...
def save_prompt_cache(file_name: str, cache: List[Any], metadata: Dict[str, str] = {}):
    ...
    cache_metadata_raw = [cache_info, metadata, cache_classes]
    cache_metadata_flat = dict(tree_flatten(cache_metadata_raw))

    # Convert metadata values to strings
    metadata_str = {k: str(v) for k, v in cache_metadata_flat.items()}

    # Convert tensors to contiguous before saving
    tensors = {}
    for k, v in cache_data_flat.items():
        if isinstance(v, torch.Tensor):
            ...
            tensors[k] = (
                torch.tensor(v) if not isinstance(v, str) else torch.tensor([0])
            )

    safetensors.torch.save_file(tensors, file_name, metadata=metadata_str)


def load_prompt_cache(file_name, return_metadata=False):
    import safetensors

    data = safetensors.torch.load_file(file_name)
    meta = safetensors.torch.load_file(file_name)  # metadata approach
    ...
    if metadata:
        cache_metadata = {k: v for k, v in metadata.items()}
        cache_metadata = tree_unflatten(list(cache_metadata.items()))
        info, md, classes = cache_metadata
        cache = [
    ...
    if return_metadata:
        return cache, metadata if metadata else {}
    return cache
...
class ConcatenateKVCache(_BaseCache):
    """Simplest KV cache - concatenates keys/values."""
...
class KVCache(_BaseCache):
    ...
    def update_and_fetch(self, keys, values):
        ...
        if self.keys is None or (prev + keys.shape[2]) > self.keys.shape[2]:
            ...
            new_k = torch.zeros(k_shape, dtype=keys.dtype, device=keys.device)
            new_v = torch.zeros(v_shape, dtype=values.dtype, device=values.device)
    ...
    def make_mask(
        ...
        if N > 1:
            ...
            if self.offset >= window_size and self.max_size > window_size:
                ...
                mask = torch.arange(mask_size) >= (mask_size - window_size)
                mask = torch.roll(mask, shifts=idx + 1)
                return mask
...
class QuantizedKVCache(_BaseCache):
    """KV cache with per-group quantization for memory efficiency."""

    def __init__(self, group_size: int = 64, bits: int = 8):
        self.keys = None
        self.values = None
        self.keys_scales = None
        self.keys_biases = None
        self.values_scales = None
        self.values_biases = None
        self.offset = 0
    ...
    def _quantize(self, x):
        """Quantize float tensor to uint8 with per-group scales and biases."""
        B, H, S, D = x.shape
        # Reshape for group quantization
        n_groups = D // self.group_size
        x_grouped = x.reshape(B, H, S, n_groups, self.group_size)
        min_val = x_grouped.min(dim=-1, keepdim=True).values
        max_val = x_grouped.max(dim=-1, keepdim=True).values
        scale = (max_val - min_val) / (2**self.bits - 1)
        bias = min_val
        # Avoid division by zero
        scale = torch.where(scale == 0, torch.ones_like(scale), scale)
        quantized = torch.round((x_grouped - bias) / scale).to(torch.uint8)
        return quantized, scale, bias

    def _dequantize(self, quantized, scales, biases):
        """Dequantize uint8 tensor back to float using scales and biases."""
        # quantized: (B, H, S, n_groups, group_size), scales/biases: (B, H, S, n_groups, 1)
        quantized_f = quantized.to(torch.float32)
        result = quantized_f * scales + biases
        B, H, S = result.shape[:3]
        return result.reshape(B, H, S, -1)
    ...
    def update_and_fetch(self, keys, values):
        ...
        qk, ks, kb = self._quantize(keys)
        qv, vs, vb = self._quantize(values)
        ...
        if self.keys is None:
            ...
            self.keys_scales = ks
            self.keys_biases = kb
            self.values_scales = vs
            self.values_biases = vb
        else:
            self.keys = torch.cat([self.keys, qk], dim=2)
            self.values = torch.cat([self.values, qv], dim=2)
            self.keys_scales = torch.cat([self.keys_scales, ks], dim=2)
            self.keys_biases = torch.cat([self.keys_biases, kb], dim=2)
            self.values_scales = torch.cat([self.values_scales, vs], dim=2)
            self.values_biases = torch.cat([self.values_biases, vb], dim=2)

        # Dequantize full cache for return
        dk = self._dequantize(self.keys, self.keys_scales, self.keys_biases)
        dv = self._dequantize(self.values, self.values_scales, self.values_biases)
        return dk[..., : self.offset, :], dv[..., : self.offset, :]
    ...
    def nbytes(self):
        ...
        return (
            self.keys.element_size() * self.keys.nelement()
            + self.values.element_size() * self.values.nelement()
            + self.keys_scales.element_size() * self.keys_scales.nelement()
            + self.keys_biases.element_size() * self.keys_biases.nelement()
            + self.values_scales.element_size() * self.values_scales.nelement()
            + self.values_biases.element_size() * self.values_biases.nelement()
        )
...
class ArraysCache(_BaseCache):
    ...
    def __init__(self, size, left_padding: Optional[List[int]] = None):
        ...
        if left_padding:
            self.left_padding = torch.tensor(left_padding)
    ...
    def extend(self, other):
        self.cache = [torch.cat([c, o]) for c, o in zip(self.cache, other.cache)]
    ...
    def merge(cls, caches):
        ...
        for e in range(n_state):
            ...
            shape[0] = B
            cache[e] = torch.zeros(shape, dtype=c_init.dtype, device=c_init.device)
            for i in range(B):
    ...
    @property
    def nbytes(self):
        return sum(c.element_size() * c.nelement() for c in self.cache if c is not None)
...
def dynamic_roll(x, shifts, axis):
    n = x.shape[axis]
    expand_shifts = (...,) + (None,) * (x.ndim - axis - 1)
    expand_indices = (...,) + (None,) * (x.ndim - axis - 1)
    idx = (torch.arange(n, device=x.device).view(expand_indices) - shifts) % n
    rolled = torch.gather(x, axis, idx.expand_as(x))
    return rolled
...
class BatchKVCache(_BaseCache):
    ...
    def __init__(self, left_padding: List[int]):
        ...
        self.values = None
        self.left_padding = torch.tensor(left_padding)
        self.offset = torch.tensor([-l for l in left_padding])
        self._idx = 0
    ...
    def prepare(self, *, left_padding=None, lengths=None, right_padding=None):
        if left_padding is not None:
            ...
            left_padding = torch.tensor(left_padding)
            self.left_padding += left_padding
            self.offset -= left_padding

        if right_padding is not None and max(right_padding) > 0:
            self._right_padding = torch.tensor(right_padding)
    ...
    def extend(self, other):
        ...
        self.keys, self.values, self.offset, self.left_padding = map(
            torch.cat, zip(*(pad(self), pad(other)))
        )
        self._idx = max_idx
    ...
    def extract(self, idx):
        ...
        cache.keys = self.keys[idx : idx + 1, :, padding : self._idx].contiguous()
        cache.values = self.values[idx : idx + 1, :, padding : self._idx].contiguous()
    ...
    def merge(cls, caches):
        ...
        keys = torch.zeros((B, H, max_length, Dk), dtype=dt)
        values = torch.zeros((B, H, max_length, Dv), dtype=dt)
...
class BatchRotatingKVCache(_BaseCache):
    ...
    def _trim(self, trim_size, v, append=None):
        if trim_size > 0:
            v = v[..., trim_size:, :]
        if append is not None:
            return torch.cat([v, append], dim=2)
        return v

    def _temporal_order(self):
        if self.rotated:
            self.keys = torch.roll(self.keys, -self._idx, dims=2)
            self.values = torch.roll(self.values, -self._idx, dims=2)
            self._idx = self.keys.shape[2]
            self.rotated = False
    ...
    def _update_concat(self, keys, values):
        if self.keys is None:
            ...
            if self._lengths is not None:
                roll = torch.maximum(
                    torch.zeros_like(self.offset), self.offset - self._lengths
                )
    ...
    def prepare(self, *, left_padding=None, lengths=None, right_padding=None):
        ...
        if right_padding is not None and max(right_padding) > 0:
            self._lengths = torch.tensor(lengths) + self.offset
    ...
    def make_mask(
        ...
        mask = mask & (rinds >= left_padding.view(1, 1, 1, -1))

        if rotated:
            idx = self._idx
            if idx >= self.max_size:
                idx = 0
            mask = torch.roll(mask, shifts=idx + 1, dims=-1)
    ...
    def extract(self, idx):
        ...
        if self.rotated:
            cache.keys = torch.roll(cache.keys, -self._idx, dims=2)
            cache.values = torch.roll(cache.values, -self._idx, dims=2)
            cache._idx = self.max_size
        cache.keys = cache.keys[:, :, padding : cache._idx].contiguous()
        cache.values = cache.values[:, :, padding : cache._idx].contiguous()
        cache.offset = offset
    ...
    def merge(cls, caches):
        ...
        cache.keys = keys
        cache.values = values
        cache.offset = torch.tensor(offsets)
        cache._idx = keys.shape[2]
