...
class Attention(nn.Module):
    ...
    def forward(
        ...
        q = q.reshape(B, L, self.num_heads, -1).transpose(1, 2)
        ...
        kv = kv.reshape(B, L, self.num_heads, -1).transpose(1, 2)

        k_nope, values = torch.split(
            kv, [self.qk_nope_head_dim, kv.shape[-1] - self.qk_nope_head_dim], dim=-1
        )
        ...
        k_pe_broadcasted = k_pe.expand(B, self.num_heads, L, self.qk_rope_head_dim)

        # Use concatenate for queries
        queries = torch.cat([q_nope, q_pe], dim=-1)

        # Use concatenate for keys
        keys = torch.cat([k_nope, k_pe_broadcasted], dim=-1)
