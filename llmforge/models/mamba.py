...
class MambaBlock(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        if self.use_bcdt_rms:
            self.mixer_norm = lambda x: (
                x * torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + args.mixer_rms_eps)
            )
        ...
        A = torch.repeat_interleave(
            torch.arange(1.0, self.ssm_state_size + 1.0).reshape(
                [1, self.ssm_state_size]
            ),
    ...
    def ssm_step(self, x, A, state=None):
        ...
        delta, B, C = map(
            ...
            torch.split(
                ...
                [
                    ...
                    deltaBC.shape[-1] - self.time_step_rank - self.ssm_state_size,
                ],
                dim=-1,
            ),
        )
        ...
        new_state = delta.unsqueeze(-1) * x.unsqueeze(-1) * B.unsqueeze(1)
        if state is not None:
            new_state = new_state + state * torch.exp(delta.unsqueeze(-1) * A)
        y = (new_state @ C.unsqueeze(-1)).squeeze(2)
        y = y + D * x
        return y, new_state
    ...
    def _process_sequence(self, x, conv_cache, state_cache):
        B, T, D = x.shape
        xz = self.in_proj(x)
        x, z = torch.split(xz, xz.shape[-1] // 2, dim=-1)
        K = self.conv_kernel_size
        if conv_cache is not None:
            x_full = torch.cat([conv_cache, x], dim=1)
        else:
            x_full = F.pad(x, (0, 0, K - 1, 0))
        conv_out = self.conv1d(x_full.transpose(1, 2)).transpose(1, 2)
...
class Mamba(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.layers = nn.ModuleList(
            [ResidualBlock(args) for _ in range(args.num_hidden_layers)]
        )
        self.norm_f = RMSNorm(args.hidden_size)
...
class Model(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, cache=None):
        ...
        if self.args.tie_word_embeddings:
            logits = F.linear(x, self.backbone.embeddings.weight)
        else:
